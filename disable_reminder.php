...
<body><div class="disable-reminder">
        <h2>Disable  Reminder</h2><br>          
       
        <form>
          <div id="message"></div>
          Select a date:
            <input type="text" name="date" id="datepicker-6" format="mm-dd-yyyy" class="form-control" value="<?php if(isset($_REQUEST['date'])) echo $_REQUEST['date'];?>"><br>
            <input type="name" id="url" class="form-control" name="url" value="<?php echo URL_ROOT."disable_reminder.php?date=";?>" hidden>

            <?php if(isset($_REQUEST['date'])){?>
              <?php 
                $sql = "SELECT * FROM manage_reminder where status=1 AND user_id=".$_SESSION['id'];
...
$("form").submit(function(event){
	$.ajax({
		url : 'ajax.php',
		type: 'POST',
		data : $(this).serialize()+"&id="+$('#reminder').val()+"&action=disable_reminder",
    dataType: 'json',
    encode : true,
	}).done(function(data){ 
    if(data.status == 1){
